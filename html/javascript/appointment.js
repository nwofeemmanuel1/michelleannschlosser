...
const submitForm=async(firstName,lastName,email,phoneNumber,country,message)=>{
...
try {
    const response=await fetch("/api/submit/appointment",{
        ...
        body:JSON.stringify({firstName,lastName,email,phoneNumber,country,message})
    })
...
submitForm(firstName.value,lastName.value,email.value,phoneNumber.value,country.value,message.value);

}

document.querySelectorAll("input").forEach(input=> input.onkeyup=()=>{
    input.style.border="2px solid gray"
}
)
document.querySelector("select").onchange=()=>document.querySelector("select").style.border="2px solid gray"
document.querySelector("textarea").onkeyup=()=>document.querySelector("textarea").style.border="2px solid gray"